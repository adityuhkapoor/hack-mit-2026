...
class NativePresenter:
    ...
    def present_rgb24(self, frame: bytes | bytearray | memoryview, *, stride: int | None = None) -> None:
        ...
        if stride < row_bytes:
            raise ValueError("stride is smaller than one RGB24 row")
        ...
        result = self._library.np_present_rgb24(
            self._handle, pointer, view.nbytes, self.width, self.height, stride)
        if result != 0:
            raise PresenterError(self._error())
