...
## Integration with current main (2026-09-20)

Integrated onto 3b81a77 for a controlled Pi deployment. Preserved f319a0c's
proportional gain/exposure calculation inside the reader-thread shutter path,
including the gain-ceiling fallback. Added both cases as regressions and updated
the older test that expected unconditional maximum gain. Current shop and app
changes are retained. Historical not-deployed statements above describe earlier
review stages; deployment status is recorded separately after hardware checks.
