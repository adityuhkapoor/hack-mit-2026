# Installs the lookcam pipeline API on the GPU box and runs it behind cloudflared.
# Source is copied to %USERPROFILE%\hackmit\pipeline by deploy_api.sh; this builds the venv and
# registers the "LookcamAPI" scheduled task. Idempotent.
...
Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
    Where-Object { $_.CommandLine -like '*lookcam.api*' } |
...
Register-ScheduledTask -TaskName "LookcamAPI" -Action $action -Trigger $trigger -Settings $settings -Force | Out-Null
Start-ScheduledTask -TaskName "LookcamAPI"
"LookcamAPI registered and started"
