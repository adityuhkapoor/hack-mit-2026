...
class CameraApp:
    def __init__(self, sensors, camera, library, api: str = API, render_locally: bool = True):
        ...
        self.state = State()
        self.http = httpx.Client(timeout=httpx.Timeout(45, connect=3), transport=httpx.HTTPTransport(retries=0))
    ...
    def _take_photo(self, p: dict | None = None) -> dict:
        ...
        if dial == VISA_BUY:                      # the saved photo is also the shopping query
            lookup = self.identify_product({"photo": photo.id})
            if "error" not in lookup:
                return lookup
            self.state.current, self.state.screen = photo, "review"
            out = self._summary(photo)
            out["note"] = "saved locally; product lookup is unavailable"
            out["product_lookup_error"] = lookup["error"]
            self.say("Saved locally · product lookup unavailable")
            return out
    ...
    def _public(self, photo: Photo) -> Photo:
        ...
        server_id = photo.remote_id or photo.id
        files = {}
        for k in ("photo", "as_shot", "mask"):
            f = d / f"{k}.jpg"
            data = f.read_bytes() if f.exists() else (self.http.get(f"{self.api}/captures/{server_id}/{k}.jpg", timeout=30).content
        ...
        meta = self.http.get(f"{self.api}/captures/{server_id}", timeout=15).json() if photo.photo_url else None
        if meta is None:
            return photo
    ...
    def print_photo(self, p: dict | None = None) -> dict:
        ...
        try:
            r = self.http.post(f"{self.api}/captures/{photo.remote_id or photo.id}/print", params=params,
    ...
    def _capture(self, dial: int) -> Photo:
        ...
        local = self._persist_original(jpeg, readings, dial)
        if dial == VISA_BUY:
            return local
        # The upload and the server's segmentation run while Muse is still choosing (~6 s): two phases.
        prepared: dict = {}
        upload_slot = getattr(self, "_upload_slot", None)
        if upload_slot is None:  # supports lightweight test rigs constructed without __init__
            upload_slot = self._upload_slot = threading.Lock()
        if not upload_slot.acquire(blocking=False):
            local.fallback_reason = "AI upload is still recovering; original saved locally"
            self._index(local)
            return local
        ...
        def upload():
            try:
                r = capture_http.request("POST", f"{self.api}/capture/prepare", phase="prepare", files={"photo": ("shot.jpg", jpeg, "image/jpeg")},
                                   data={"readings": json.dumps(readings)}, timeout=httpx.Timeout(15, connect=3))
                r.raise_for_status()
        ...
        if sv.get("selection_failed"):
            local.fallback_reason = "Scene recognition unavailable; original saved instead of guessing a design"
            self._index(local)
            return local
        ...
        if not prepared.get("id"):
            local.fallback_reason = "AI server unavailable before generation; original saved locally"
            self._index(local)
            return local
        try:
            # Never retry this POST. A lost response does not prove whether generation ran.
            r = capture_http.request("POST", f"{self.api}/capture/finish", phase="finish",
                                     timeout=httpx.Timeout(150, connect=3),
                                     data={"prepared": prepared["id"], "dial": str(sense.SOUVENIR),
                                           "seed": seed, "souvenir": json.dumps(sv)})
            return self._store_server(r.json(), existing=local)
        except Exception as e:
            local.fallback_reason = f"AI generation unavailable ({type(e).__name__}); original saved locally"
            self._index(local)
            return local

    def _persist_original(self, jpeg: bytes, readings: dict, dial: int) -> Photo:
        """Durably store the shutter bytes and local metadata before any network or model call."""
        prefix = "buy" if dial == VISA_BUY else "cam"
        pid = prefix + datetime.now().strftime("%Y%m%d%H%M%S%f") + uuid.uuid4().hex[:6]
        d = HOME / "photos" / pid
        d.mkdir(parents=True, exist_ok=False)
        original = d / "original.jpg"
        tmp = d / ".original.jpg.tmp"
        with tmp.open("wb") as stream:
            stream.write(jpeg)
            stream.flush()
            os.fsync(stream.fileno())
        os.replace(tmp, original)
        directory = os.open(d, os.O_RDONLY)
        try:
            os.fsync(directory)
        finally:
            os.close(directory)
        # The gallery always has something displayable, even if every enhancement is unavailable.
        (d / "photo.jpg").write_bytes(jpeg)
        name = DIALS[dial]
        photo = Photo(id=pid, created_at=_now_iso(), dial=dial, dial_name=name, readings=dict(readings),
                      untouched=True, proof="as shot", local_photo=str(d / "photo.jpg"),
                      local_original=str(original), processed_on="camera")
        base = tagger.from_readings(photo.readings, name)
        photo.caption, photo.tags = base["caption"], base["tags"]
        (d / "capture.json").write_text(json.dumps({"photo": photo.__dict__}, indent=2))
        self._index(photo)
        return photo

    def _index(self, photo: Photo) -> None:
        """Indexing enriches a saved file; it cannot turn a successful shutter into a failure."""
        if photo.local_photo:
            metadata = Path(photo.local_photo).parent / "capture.json"
            tmp = metadata.with_suffix(".json.tmp")
            try:
                tmp.write_text(json.dumps({"photo": photo.__dict__}, indent=2))
                os.replace(tmp, metadata)
            except OSError as e:
                print(f"[library] metadata update failed ({type(e).__name__})")
        try:
            self.library.add(photo)
        except Exception as e:
            print(f"[library] index failed ({type(e).__name__}); photo remains on disk")
    ...
    def _capture_plain(self, jpeg: bytes, readings: dict) -> Photo:
        ...
        return self._persist_original(jpeg, readings, VISA_BUY)
    ...
    def _store_server(self, meta: dict, files: dict[str, bytes] | None = None,
                      existing: Photo | None = None, fallback_photo: bytes | None = None) -> Photo:
        pid = meta["id"]
        d = Path(existing.local_photo).parent if existing and existing.local_photo else HOME / "photos" / pid
        d.mkdir(parents=True, exist_ok=True)
        try:
            photo_jpg = (files or {}).get("photo") or capture_http.request(
                "GET", f"{self.api}/captures/{pid}/photo.jpg", phase="photo-download",
                timeout=httpx.Timeout(5, connect=3)).content
        except capture_http.CaptureRequestError:
            if existing and existing.local_original:
                fallback_photo = Path(existing.local_original).read_bytes()
            if fallback_photo is None:
                raise
            # Generation completed and remains on the server. Keep the original
            # usable locally when the Pi→ASUS tunnel drops during the final GET.
            photo_jpg = fallback_photo
            meta = dict(meta)
            meta["processed_on"] = "camera"
            meta["fallback_reason"] = "Original saved; generated copy remains on server"
            meta["untouched"] = True
            meta["proof"] = "original saved; generated copy pending"
            print(f"[capture http] phase=photo-download fallback=original capture_id={pid}")
        (d / "photo.jpg").write_bytes(photo_jpg)
        taken = datetime.fromtimestamp(meta["created_at"]).astimezone().isoformat(timespec="seconds")
        photo = existing or Photo(id=pid, created_at=taken, dial=meta["dial_used"], dial_name=meta["dial_name"],
                                  readings=meta["readings"])
        photo.created_at, photo.dial, photo.dial_name = taken, meta["dial_used"], meta["dial_name"]
        photo.readings, photo.web = meta["readings"], meta.get("web", [])
        photo.untouched, photo.proof = meta["untouched"], meta["proof"]
        photo.photo_url, photo.card_url, photo.link = (f"{self.api}/captures/{pid}/photo.jpg",
                                                       f"{self.api}/captures/{pid}/card.jpg", f"{self.api}/c/{pid}")
        photo.local_photo, photo.processed_on = str(d / "photo.jpg"), meta.get("processed_on", "server")
        photo.remote_id, photo.fallback_reason = pid, meta.get("fallback_reason")
        base = tagger.from_readings(photo.readings, photo.dial_name)
        photo.caption, photo.tags = base["caption"], base["tags"]
        self._index(photo)       # searchable immediately, by time and air; Muse's tags follow
        return photo
