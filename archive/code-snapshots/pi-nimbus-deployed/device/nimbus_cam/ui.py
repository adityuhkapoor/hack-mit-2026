    ...
    def __init__(self, app: CameraApp, voice=None, fullscreen: bool | None = None):
        ...
        if os.environ.get("NIMBUS_UI_BACKEND") == "sdl":
            try:
                ...
                # NIMBUS_SDL_PIXELS=rgbx uploads 4-byte rows the GL driver takes as-is (see presenter.py).
                self._native = NativePresenter(self.W, self.H, fullscreen=full,
                                               vsync=False, require_accelerated=True,
                                               pixel_format=os.environ.get("NIMBUS_SDL_PIXELS", "rgb24"))
    ...
    def _tick(self) -> None:
        ...
        try:
            ...
            if native is not None:
                ...
                if stats is not None and hasattr(native, "last_timing"):
                    stats.record_native(native.last_timing())
            else:
