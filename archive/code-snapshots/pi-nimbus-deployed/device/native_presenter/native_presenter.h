...
enum np_create_flags {
    ...
    /* Texture and frames are 4 bytes per pixel, R G B X in memory (SDL ABGR8888, GL_RGBA), instead of
     * RGB24. GLES drivers have no 24-bit texture format, so an RGB24 upload is repacked on the CPU
     * before it is tiled; a 4-byte frame is uploaded as-is. Frames must then come from present_rgbx32. */
    NP_PIXELS_RGBX = 1u << 4,
};
...
typedef struct np_renderer_info {
    ...
    uint8_t bytes_per_pixel;   /* 3 (RGB24) or 4 (RGBX, NP_PIXELS_RGBX) */
    uint8_t reserved[3];
} np_renderer_info;
...
/*
 * The same for a presenter created with NP_PIXELS_RGBX: 4 bytes per pixel, R G B X in memory, the
 * fourth byte ignored. buffer_length must cover (height - 1) * stride + width * 4.
 */
NP_API int np_present_rgbx32(np_presenter *presenter, const void *pixels,
                             size_t buffer_length, int32_t width, int32_t height,
                             int32_t stride);
