...
`--pixels rgbx` presents 4-byte RGBX rows into an RGBA texture instead of RGB24
(`NIMBUS_SDL_PIXELS=rgbx` does the same in the camera UI). GLES drivers have no
24-bit texture format, so an RGB24 upload is repacked on the CPU before it is
tiled; RGBX is uploaded as-is but carries a third more bytes. Which wins on the
Pi's V3D driver is a measurement to take, not a claim. The camera's frame
stats now also print the presenter's own upload/present split.
