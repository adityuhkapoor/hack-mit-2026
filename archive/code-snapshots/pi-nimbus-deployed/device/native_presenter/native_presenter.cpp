...
np_presenter *np_create(int32_t texture_width, int32_t texture_height,
    ...
    const int bytes_per_pixel = (flags & NP_PIXELS_RGBX) ? 4 : 3;
    if (texture_width > std::numeric_limits<int>::max() / bytes_per_pixel) {
        set_error("texture width is too large for the pixel format");
        return nullptr;
    }
    if (!acquire_video()) {
        return nullptr;
    ...
    presenter->bytes_per_pixel = bytes_per_pixel;
    ...
    /* ABGR8888 is the packed name; in memory on a little-endian machine it is R, G, B, A: what Pillow's
     * "RGBX" raw packer writes and what GL_RGBA/GL_UNSIGNED_BYTE consumes without repacking. */
    const Uint32 pixel_format = bytes_per_pixel == 4 ? SDL_PIXELFORMAT_ABGR8888 : SDL_PIXELFORMAT_RGB24;
    presenter->texture = SDL_CreateTexture(presenter->renderer, pixel_format,
                                           SDL_TEXTUREACCESS_STREAMING,
                                           texture_width, texture_height);
    if (!presenter->texture) {
        set_sdl_error("SDL_CreateTexture(streaming)");
        cleanup(presenter);
        return nullptr;
    }
    /* The X byte carries whatever Pillow left there; a format with alpha defaults to blending on it. */
    if (SDL_SetTextureBlendMode(presenter->texture, SDL_BLENDMODE_NONE) != 0) {
        set_sdl_error("SDL_SetTextureBlendMode");
        cleanup(presenter);
        return nullptr;
    }
...
int present_frame(np_presenter *presenter, const void *pixels, size_t buffer_length,
                  int32_t width, int32_t height, int32_t stride, int bytes_per_pixel) {
    last_error.clear();
    if (!on_owner_thread(presenter)) {
        return -1;
    }
    ...
    if (bytes_per_pixel != presenter->bytes_per_pixel) {
        set_error(presenter->bytes_per_pixel == 4 ? "presenter was created for RGBX frames"
                                                  : "presenter was created for RGB24 frames");
        return -1;
    }
    ...
    if (width <= 0 || height <= 0 || width > std::numeric_limits<int>::max() / bytes_per_pixel) {
        set_error("invalid frame dimensions");
        return -1;
    }
    ...
    if (stride < 0 || static_cast<size_t>(stride) < row_bytes) {
        set_error("stride is smaller than one row");
        return -1;
    }
    ...
    if (texture_pitch < 0 || static_cast<size_t>(texture_pitch) < row_bytes) {
        ...
        set_error("SDL returned a texture pitch smaller than one row");
        return -1;
    }
...
int np_present_rgb24(np_presenter *presenter, const void *pixels,
                     ...
                     int32_t stride) {
    return present_frame(presenter, pixels, buffer_length, width, height, stride, 3);
}

int np_present_rgbx32(np_presenter *presenter, const void *pixels,
                      size_t buffer_length, int32_t width, int32_t height,
                      int32_t stride) {
    return present_frame(presenter, pixels, buffer_length, width, height, stride, 4);
}
...
int np_get_renderer_info(np_presenter *presenter, np_renderer_info *info_out) {
    ...
    info_out->bytes_per_pixel = static_cast<uint8_t>(presenter->bytes_per_pixel);
    return 0;
}
